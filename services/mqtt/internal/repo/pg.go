...
func (c *ConstructionPGRepository) Update(ctx *gofr.Context, data *usecase.ConstructionEntity) (bool, error) {
	...
	subSqlStr := "UPDATE mqtt_construction_running SET name = $1,container_id = $2,comments = $3, construction_id = $4 WHERE id = $5;"
	...
	for _, r := range data.Runners {
		_, err = tx.ExecContext(ctx, subSqlStr, r.Name, r.ContainerID, r.Comments, data.ID, r.ID)
		if err != nil {
			tx.Rollback()
